    ...
    m_carriersConfigured (false)
{
  ...
  //Ravi URLLC Sap user
  m_ffrRrcUrllcSapUser.push_back (new MemberLteFfrRrcSapUser<LteEnbRrc> (this));
...
LteEnbRrc::DoDispose ()
{
  ...
  //URLLC delete
  m_ffrRrcUrllcSapUser.erase (m_ffrRrcUrllcSapUser.begin (),m_ffrRrcUrllcSapUser.end ());
  m_ffrRrcUrllcSapUser.clear ();
  m_ueMap.clear ();  
...
//Ravi provider for URLLC
void
LteEnbRrc::SetLteFfrRrcUrllcSapProvider (LteFfrRrcSapProvider * s, uint8_t index)
{
  NS_LOG_FUNCTION (this << s);
  if (m_ffrRrcUrllcSapProvider.size () > index)
    {
      m_ffrRrcUrllcSapProvider.at (index) = s;
    }
  else
    {
      m_ffrRrcUrllcSapProvider.push_back (s);
      NS_ABORT_MSG_IF (m_ffrRrcUrllcSapProvider.size () - 1 != index,
                       "You meant to store the pointer at position " <<
                       static_cast<uint32_t> (index) <<
                       " but it went to " <<m_ffrRrcUrllcSapProvider.size () - 1);
    }
...
//Ravi URLLC Get Sap User
LteFfrRrcSapUser*
LteEnbRrc::GetLteFfrRrcUrllcSapUser (uint8_t index)
{
  NS_LOG_FUNCTION (this);
  ...
  return m_ffrRrcUrllcSapUser.at (index);
}
  ...
  for (const auto &it: ccPhyConf)
    {
      ...
      if (m_ffrRrcSapProvider.size () > it.first)
        ...
        if (m_ffrRrcUrllcSapProvider.size () > it.first)
        {
          m_ffrRrcUrllcSapProvider.at (it.first)->SetCellId (it.second->GetCellId ());
          m_ffrRrcUrllcSapProvider.at (it.first)->SetBandwidth (it.second->GetUlBandwidth (), it.second->GetDlBandwidth ());
        }
    }
