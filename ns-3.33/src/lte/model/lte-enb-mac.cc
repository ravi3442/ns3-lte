...
void
EnbMacMemberLteEnbCmacSapProvider::ConfigureMac (uint16_t _ulBandwidth, uint16_t _dlBandwidth)
{
  m_mac->ulBandwidth = _ulBandwidth;
  m_mac->dlBandwidth = _dlBandwidth;
  m_mac->DoConfigureMac (_ulBandwidth, _dlBandwidth);
}
...
LteEnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteEnbMac")
    ...
    .AddTraceSource ("RbgAllocation",
                     "Regarding Embb and URLLC Rbg Allocation.",
                     MakeTraceSourceAccessor (&LteEnbMac::rbgAllocation),
                     "ns3::LteEnbMac::rbgAllocationTracedCallback")
...
m_ccmMacSapUser (0)
{
  ...
  //URLLC
  m_schedSapUrllcUser= new EnbMacMemberFfMacSchedSapUser (this);
  m_cschedSapUrllcUser = new EnbMacMemberFfMacCschedSapUser (this); //FfMacCschedSapUser

  //Ravi AMC
  m_amc= CreateObject <LteAmc> (); ///< AMC

  //Execute RB management
  ExecuteRBManagement();
}
...
//Ravi
void
LteEnbMac::SetFfMacUrllcSchedSapProvider (FfMacSchedSapProvider* s)
{
  m_schedSapUrllcProvider = s;
}
...
//Ravi
FfMacSchedSapUser*
LteEnbMac::GetFfMacUrllcSchedSapUser (void)
{
  return m_schedSapUrllcUser;
}
...
void
LteEnbMac::SetFfMacCschedUrllcSapProvider (FfMacCschedSapProvider* s)
{
  m_cschedSapUrllcProvider = s;
}
...
//Ravi
FfMacCschedSapUser*
LteEnbMac::GetFfMacCschedUrllcSapUser (void)
{
  return m_cschedSapUrllcUser;
}
...
//Ravi
  if (m_dlCqiReceived.size () > 0 || m_dlCqiUrllcReceived.size ()>0)
    {

      std::vector <CqiListElement_s>::iterator it;
      FfMacSchedSapProvider::SchedDlCqiInfoReqParameters dlcqiInfoReq;
      FfMacSchedSapProvider::SchedDlCqiInfoReqParameters dlcqiUrllcInfoReq;

      for(it = m_dlCqiReceived.begin (); it != m_dlCqiReceived.end() ; it++){

        if(it->m_rnti ==2 ){
          NS_LOG_INFO("DL CQI for Embb");
          dlcqiInfoReq.m_sfnSf = ((0x3FF & frameNo) << 4) | (0xF & subframeNo);
          dlcqiInfoReq.m_cqiList.push_back(*it);
          
      }else if (it->m_rnti==1){
          NS_LOG_INFO("DL CQI for URLLC");
          dlcqiUrllcInfoReq.m_sfnSf = ((0x3FF & frameNo) << 4) | (0xF & subframeNo);
          dlcqiUrllcInfoReq.m_cqiList.push_back(*it);
      }
    }

    for(it = m_dlCqiUrllcReceived.begin (); it != m_dlCqiUrllcReceived.end() ; it++){
    ...
    m_dlCqiReceived.erase (m_dlCqiReceived.begin (), m_dlCqiReceived.end ());
    m_dlCqiUrllcReceived.erase (m_dlCqiUrllcReceived.begin (), m_dlCqiUrllcReceived.end ());
    m_schedSapProvider->SchedDlCqiInfoReq (dlcqiInfoReq);
    m_schedSapUrllcProvider->SchedDlCqiInfoReq (dlcqiUrllcInfoReq);
  ...
  //Ravi
  m_schedSapUrllcProvider->SchedDlTriggerReq (dlparams);
...
LteEnbMac::ReceiveDlCqiLteControlMessage  (Ptr<DlCqiLteControlMessage> msg)
{
  ...
  if(dlcqi.m_rnti==1){
    NS_LOG_LOGIC (this << "URLLC Enb Received DL-CQI rnti" );
    m_dlCqiUrllcReceived.push_back (dlcqi);
  }else if(dlcqi.m_rnti==2){
    NS_LOG_LOGIC (this << "EMBB Enb Received DL-CQI rnti" );
...
LteEnbMac::DoConfigureMac (uint16_t ulBandwidth, uint16_t dlBandwidth)
{
  ...
  //Ravi Set parameters for the URLLC Schedular
  m_cschedSapUrllcProvider->CschedCellConfigReq (params);
}
...
LteEnbMac::DoAddUe (uint16_t rnti)
{
  ...
  if ( rnti == 2){
    ...
    m_cschedSapUrllcProvider->CschedUeConfigReq (params);
  }
...
//Ravi
      ...
      if(params.m_rnti==1){
        m_cschedSapUrllcProvider->CschedLcConfigReq (params);
      }
...
LteEnbMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  NS_LOG_INFO("********************");
  NS_LOG_INFO("Rnti: "<<params.rnti);
  NS_LOG_INFO("Tx Queue Size: "<<params.txQueueSize);
  NS_LOG_INFO("TX Queue HOL Delay: "<<params.txQueueHolDelay);
  NS_LOG_INFO("Re Tx Queue Size: "<<params.retxQueueSize);
  NS_LOG_INFO("Re Tx Queue Hol Delay: "<<params.retxQueueHolDelay);
  NS_LOG_INFO("RLC Status PDU Size: "<<params.statusPduSize);
  NS_LOG_INFO("New Data Arrival: "<<params.newDataSize);
  NS_LOG_INFO("********************");

//Ravi , add the statistics to a data structure

arrivalData dat, prevData;

  if(req.m_rnti==2){
    
    //new data
    dat.rnti = params.rnti;
    dat.bufferSize = params.txQueueSize;
    dat.holDelay = params.txQueueHolDelay;
    dat.timeStamp = Simulator::Now();
    
    if(userStats[params.rnti].size()==0){

          dat.newArrival = params.txQueueSize;
          userStats[params.rnti].push_front(dat);
          dat.newArrival = params.txQueueSize;
          

          //initialize it once
          userAvgStats[params.rnti].averageArrivalInBytes=(uint32_t)99999;
          userAvgStats[params.rnti].buffersizeInBytes=(uint16_t)99999;
          SchedlastUpdate[params.rnti].lastMcs =-1;
          SchedlastUpdate[params.rnti].lastTbs =-1; 
          SchedlastUpdate[params.rnti].RbBitMap=-1;

    }else{
      //check if this is the first entry, else compare it with the previous entry
       //previous entry
      prevData = userStats[params.rnti].front();

      int16_t newData = params.txQueueSize - prevData.bufferSize;

      NS_LOG_INFO("RNTI "<<params.rnti<<" Current QueuSize:  "<<params.txQueueSize<<" Prev Buffer "<<prevData.bufferSize);

      if(newData <0){
        dat.newArrival=0;
      }else dat.newArrival =newData;

      if(dat.newArrival ==0 && params.newDataSize==0){
        NS_LOG_INFO("RNTI "<<params.rnti<<" No new data ");
        
      }else if (dat.newArrival ==0 && params.newDataSize > 0){

        NS_LOG_INFO("RNTI "<<params.rnti<<" Queuefull, new Data Size "<<dat.newArrival);
        dat.newArrival = params.newDataSize;

      }else if(dat.newArrival >0){
        NS_LOG_INFO("RNTI "<<params.rnti<<" New data arrival from prev:  "<<dat.newArrival<<" New Data Size "<<params.newDataSize);

      }
      //push the new entry
      userStats[params.rnti].push_front(dat);

      //limit the queue size to last 10 entries
      if(userStats[params.rnti].size() > 10){

        std::deque<arrivalData>::iterator it= userStats[params.rnti].begin();

        NS_LOG_INFO("****EMBB CURRENT ENTRIES****");
        for(;it!=userStats[params.rnti].end(); it++){

           NS_LOG_INFO("RNTI "<<it->rnti<<" Buffer Size "<<it->bufferSize<<" New Data Arrival "<<it->newArrival);
        }
        NS_LOG_INFO("****END****");

        userStats[params.rnti].pop_back();
    ...
    if(userStats[params.rnti].size()==0){
      ...
      if(dat.newArrival ==0 && params.newDataSize==0){
        ...
        NS_LOG_INFO("RNTI "<<params.rnti<<" Queuefull, new Data Size "<<params.newDataSize);
        dat.newArrival = params.newDataSize;
      ...
      if(userStats[params.rnti].size() > 10){
        ...
        NS_LOG_INFO("****URLLC CURRENT ENTRIES****");
        for(;it!=userStats[params.rnti].end(); it++){
   ...
   m_schedSapUrllcProvider->SchedDlRlcBufferReq (req);
  }
}


/* Ravi : RB Management, exectutes very 10 TTIs*/
void LteEnbMac::ExecuteRBManagement(){

  NS_LOG_FUNCTION (this);

  uint32_t reservedEmbbRbgs = getBlockedUrllcdRB();
  double ttiSize =0.005; uint16_t ttiDuration =5;
  int m_timeWindow= 99;
  int rbgSize =2;
  int16_t amcTbSize;
  //Normalize the current buffer size and x*tti avg. traffic to number of PRBs
  //use a sched SAP for that, or use the indication from the last schedular update

  auto itRnti= userStats.begin(); 

  //process the availb

  for( ; itRnti!= userStats.end(); itRnti++){

      uint32_t avgTempArrival =0;

      //std::deque<arrivalData>::iterator it = userStats[it->first].begin();

      auto itStats = userStats[itRnti->first].begin();

      //average the latest stats
      for (; itStats != userStats[itRnti->first].end(); itStats++){
        avgTempArrival += itStats->newArrival;
      }
      uint16_t aggBytes = avgTempArrival;
      //calculate the Bytes/s by dividing it the two times
      Time start = userStats[itRnti->first].front().timeStamp;
      Time end = userStats[itRnti->first].back().timeStamp;
      
      double bitRate = (avgTempArrival)/(start.GetSeconds() - end.GetSeconds() );

      avgTempArrival = (uint32_t)bitRate*(ttiSize/ttiDuration);

      NS_LOG_INFO("Aggreate bytes "<<aggBytes<<" Time Measured "<<(start.GetSeconds() - end.GetSeconds())<<" Byte Rate :"<<bitRate<<" Bytes/s");

      NS_LOG_INFO("ESTIMATED AVERAGE ARRIVAL PER TTI is "<<avgTempArrival);

      if(userAvgStats[itRnti->first].averageArrivalInBytes==99999){
        //initialize this to the initial average arrival from the stats        
        userAvgStats[itRnti->first].averageArrivalInBytes = avgTempArrival;
        NS_LOG_INFO("Initializing the average value to "<<userAvgStats[itRnti->first].averageArrivalInBytes);

      }else{
        //exponentially average the average arrival
       userAvgStats[itRnti->first].averageArrivalInBytes= (1.0 - (1.0 / m_timeWindow)) * userAvgStats[itRnti->first].averageArrivalInBytes + (1.0 / m_timeWindow) * avgTempArrival;
       NS_LOG_INFO("After expo. average "<<userAvgStats[itRnti->first].averageArrivalInBytes);

      }

      //record the last buffer size
      userAvgStats[itRnti->first].buffersizeInBytes = userStats[itRnti->first].front().bufferSize;
      userAvgStats[itRnti->first].holDelay=userStats[itRnti->first].front().holDelay;
      userAvgStats[itRnti->first].latestMcs = SchedlastUpdate[itRnti->first].lastMcs;
      userAvgStats[itRnti->first].latestTbs = SchedlastUpdate[itRnti->first].lastTbs;
      userAvgStats[itRnti->first].latestRbBitMap = SchedlastUpdate[itRnti->first].RbBitMap;

      if(userAvgStats[itRnti->first].latestMcs == -1) {
        NS_LOG_INFO("RNTI "<<itRnti->first<< " MCS not set ");
        Simulator::Schedule (Seconds(ttiSize), &LteEnbMac::ExecuteRBManagement, this);
        return;
      }

      amcTbSize= m_amc->GetDlTbSizeFromMcs (userAvgStats[itRnti->first].latestMcs, rbgSize) / 8;

      NS_LOG_INFO("RNTI "<<itRnti->first<< "TB SIZE per RBG with MCS "<<userAvgStats[itRnti->first].latestMcs <<"is "<<amcTbSize);

      userAvgStats[itRnti->first].averageArrivalInPrb = userAvgStats[itRnti->first].averageArrivalInBytes/amcTbSize;
      userAvgStats[itRnti->first].buffersizeInPrb = userAvgStats[itRnti->first].buffersizeInBytes/amcTbSize;

      //calculate the RBGs used 
      int numRBGs=0;
      int32_t tempVal = userAvgStats[itRnti->first].latestRbBitMap;
    
      for (uint16_t k = 1; k <= 32; k++)
        {
          if(tempVal & (1<<k) ){
            numRBGs++;
          }
      ...
      userAvgStats[itRnti->first].numberOfRbs= numRBGs;
      //NS_LOG_INFO("RNTI "<<itRnti->first<< " # of RBGs: "<<numRBGs);

  }

  NS_LOG_INFO("******Average Statistics****");

  auto it= userAvgStats.begin();
  int32_t totalDemand =0;

  for(; it!= userAvgStats.end(); it++){

    NS_LOG_INFO("RNTI : "<<it->first);
    NS_LOG_INFO("Average Arrival (B):  "<<it->second.averageArrivalInBytes);
    NS_LOG_INFO("Average Arrival (PRB):  "<<it->second.averageArrivalInPrb);
    NS_LOG_INFO("Buffer Size(B): "<< it->second.buffersizeInBytes);
    NS_LOG_INFO("Buffer Size(PRB): "<< it->second.buffersizeInPrb);
    NS_LOG_INFO("Hol delay: "<< it->second.holDelay);
    NS_LOG_INFO("MCS : "<< it->second.latestMcs);
    NS_LOG_INFO("Tbs : "<< it->second.latestTbs);
    NS_LOG_INFO("BitMap : "<< it->second.latestRbBitMap);
    NS_LOG_INFO("# of RBs : "<<it->second.numberOfRbs);
    //NS_LOG_INFO("\n");

    
  //Normalize the bytes to the PRB requiremen

  //predict the number of TTIs
  int32_t residualBuffer = it->second.buffersizeInPrb;
  int32_t demandPerTti = it->second.averageArrivalInPrb;
  int32_t resourceGranted = it->second.numberOfRbs;

  for(int i =0 ; i<ttiDuration; i++){
    //NS_LOG_INFO(" Demand "<< residualBuffer<<" Resource Granted "<<resourceGranted<<" Demand per TTI "<< it->second.averageArrivalInPrb);
    residualBuffer = ((residualBuffer - resourceGranted )>0) ? (residualBuffer - resourceGranted ):0;
    residualBuffer = residualBuffer + demandPerTti; 
  }

  NS_LOG_INFO("RNTI "<<it->first<<" Predicted demand in PRB "<<residualBuffer<<std::endl);

  userAvgStats[it->first].predictedRbs= residualBuffer;

  totalDemand += userAvgStats[it->first].predictedRbs;
  ...
  //modify this to account for globally reserved RBGs for embb
  if(userAvgStats.size()>1){

    int embbRnti = 2;
    int urllcRnti =1;
    int32_t urllcAllocationRbg=0;
    int32_t embbAllocationRbg=0;
    int32_t maxResidue;
    int32_t dlRbg = dlBandwidth/rbgSize;

    int32_t residualPrb= ((dlRbg - userAvgStats[urllcRnti].predictedRbs )>=0)? (dlRbg - userAvgStats[urllcRnti].predictedRbs):0;

    if(residualPrb >= userAvgStats[embbRnti].predictedRbs ){
      NS_LOG_INFO("Embb demand "<<userAvgStats[embbRnti].predictedRbs <<" less than residual capacity "<<residualPrb);
      //sufficient for both the demands 
      urllcAllocationRbg = userAvgStats[urllcRnti].predictedRbs;
      embbAllocationRbg = userAvgStats[embbRnti].predictedRbs;

    }else if( residualPrb < userAvgStats[embbRnti].predictedRbs){
      NS_LOG_INFO("Total demand "<<totalDemand <<" Exceeds total Capacity "<<dlRbg);
      NS_LOG_INFO("Blocked Embb RBGs are "<<getBlockedUrllcdRB());
      maxResidue= (reservedEmbbRbgs>residualPrb)? reservedEmbbRbgs :residualPrb;

      //Check if the actual demand is less than maxResidue
      if (userAvgStats[embbRnti].predictedRbs < maxResidue)
        maxResidue = userAvgStats[embbRnti].predictedRbs;
      urllcAllocationRbg= dlRbg -  maxResidue;
      embbAllocationRbg =  maxResidue;
    }

    NS_LOG_INFO("*******FINAL ALLOCATIONS************");
    NS_LOG_INFO(" URLLC PRB "<<urllcAllocationRbg<< " Embb Allocation "<<embbAllocationRbg<<std::endl);
    NS_LOG_INFO("******************************");

    

    //Set the URLLC blocked to Embb reseved and vice-versa
    m_schedSapProvider->SetBlockedRbg(urllcAllocationRbg);
    m_schedSapUrllcProvider->SetBlockedRbg(embbAllocationRbg);

    //Ravi, trace call back for the new allocation
    //rbgCallbackInfo rbgAlloc;

    //rbgAlloc.embbRbgAllocation=embbAllocationRbg;
    //rbgAlloc.urllcRbgAllocation=urllcAllocationRbg;

    rbgAllocation(embbAllocationRbg, urllcAllocationRbg, userAvgStats[embbRnti].holDelay, userAvgStats[urllcRnti].holDelay);
  }
              ...
              if (ind.m_buildDataList.at (i).m_dci.m_ndi.at (k) == 1)
                {
                  ...
                  //Ravi ,update the stats
                  //Ravi, update the stats structure
              /* userStats[txOpParams.rnti].front().lastMcs = ind.m_buildDataList.at (i).m_dci.m_mcs.at (0);
                userStats[txOpParams.rnti].front().lastTbs = ind.m_buildDataList.at (i).m_dci.m_tbsSize.at (0);
                userStats[txOpParams.rnti].front().RbBitMap = ind.m_buildDataList.at (i).m_dci.m_rbBitmap;*/

                SchedlastUpdate[rnti].lastMcs = ind.m_buildDataList.at (i).m_dci.m_mcs.at (0);
                SchedlastUpdate[rnti].lastTbs = ind.m_buildDataList.at (i).m_dci.m_tbsSize.at (0);
                SchedlastUpdate[rnti].RbBitMap = ind.m_buildDataList.at (i).m_dci.m_rbBitmap;
                SchedlastUpdate[rnti].dlBandwidth = ind.m_buildDataList.at (i).DlBandwidth;
                SchedlastUpdate[rnti].lastRbgSize = ind.m_buildDataList.at (i).rbgSize;
