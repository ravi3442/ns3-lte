...
public:
  ...
  //Ravi
  virtual void SetBlockedRbg (const uint16_t blockedRbs)=0;
...
public:
  ...
  //Ravi
  virtual void SetBlockedRbg (const uint16_t blockedRbs);
...
//Ravi to set the blocked rbgs
template <class C>
void
MemberSchedSapProvider<C>::SetBlockedRbg (const uint16_t blockedRbs)
{
  m_scheduler->SetBlockedRB (blockedRbs);
}
