...
//Ravi
uint16_t FfMacScheduler::GetBlockedRB (){
    NS_LOG_FUNCTION (this);
    NS_LOG_FUNCTION ("Getting blocked RB "<< blockedRB);
    return blockedRB;
  }

void FfMacScheduler::SetBlockedRB( const uint16_t _blockedRB){
    NS_LOG_FUNCTION (this << "Setting blocked RB "<< _blockedRB);
    blockedRB =_blockedRB;
  }
