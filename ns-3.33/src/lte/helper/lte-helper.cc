...
TypeId LteHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("Scheduler",
                   ...
                   MakeStringChecker ())
    .AddAttribute ("UrllcScheduler",
                   "The type of scheduler to be used for eNBs. "
                   "The allowed values for this attributes are the type names "
                   "of any class inheriting from ns3::FfMacScheduler.",
                   StringValue ("ns3::PfFfMacUrllcScheduler"),
                   MakeStringAccessor (&LteHelper::SetUrllcSchedulerType,
                                       &LteHelper::GetUrllcSchedulerType),
                   MakeStringChecker ())              
...
void 
LteHelper::SetUrllcSchedulerType (std::string type) 
{
  NS_LOG_FUNCTION (this << type);
  m_schedulerUrllcFactory = ObjectFactory ();
  m_schedulerUrllcFactory.SetTypeId (type);
}
...
std::string
LteHelper::GetUrllcSchedulerType () const
{
  return m_schedulerUrllcFactory.GetTypeId ().GetName ();
} 
  ...
  for (auto it = ccMap.begin (); it != ccMap.end (); ++it)
    {
      ...
      //Ravi
      Ptr<FfMacScheduler> urllcSched = m_schedulerUrllcFactory.Create<FfMacScheduler> ();


      Ptr<LteFfrAlgorithm> ffrAlgorithm = m_ffrAlgorithmFactory.Create<LteFfrAlgorithm> ();

    //Ravi
      Ptr<LteFfrAlgorithm> ffrUrllcAlgorithm = m_ffrAlgorithmFactory.Create<LteFfrAlgorithm> ();

      DynamicCast<ComponentCarrierEnb> (it->second)->SetMac (mac);
      DynamicCast<ComponentCarrierEnb> (it->second)->SetFfMacScheduler (sched);

      //Ravi, set urllc schdular for the MAC
      DynamicCast<ComponentCarrierEnb> (it->second)->SetFfMacUrllcScheduler (urllcSched);

      DynamicCast<ComponentCarrierEnb> (it->second)->SetFfrAlgorithm (ffrAlgorithm);

    //Ravi
      DynamicCast<ComponentCarrierEnb> (it->second)->SetFfrUrllcAlgorithm(ffrUrllcAlgorithm);

      DynamicCast<ComponentCarrierEnb> (it->second)->SetPhy (phy);


    int blockedUrllcRB =0;
    int blockedEmbbRB =0;

      //RBGs that URLLC schedular cannot use, max RBG=12
      urllcSched->SetBlockedRB(blockedUrllcRB);

      //RBGs that EMBB scheduler cannot use
      sched->SetBlockedRB(blockedEmbbRB);

      //Ravi: Set the same in the mac class too

      mac->SetBlockedUrllcRB(blockedUrllcRB);

      mac->SetBlockedEmbbRB(blockedEmbbRB);
    }
  ...
  for (auto it = ccMap.begin (); it != ccMap.end (); ++it)
    {
      ...
      //Ravi URLLC SAP
      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->SetLteFfrSapProvider (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrUrllcAlgorithm ()->GetLteFfrSapProvider ());
      ...
      //Ravi set URLLC Algo SAP user
      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrUrllcAlgorithm ()->SetLteFfrSapUser (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->GetLteFfrSapUser ());


      rrc->SetLteFfrRrcSapProvider (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrAlgorithm ()->GetLteFfrRrcSapProvider (), it->first);

      //rrc to URLLC lteffrAlgo
      rrc->SetLteFfrRrcUrllcSapProvider (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrUrllcAlgorithm ()->GetLteFfrRrcSapProvider (), it->first);

      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrAlgorithm ()->SetLteFfrRrcSapUser (rrc->GetLteFfrRrcSapUser (it->first));
      //FFR SAP END

      //SAP end from URLLC to RRC
      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfrUrllcAlgorithm()->SetLteFfrRrcSapUser (rrc->GetLteFfrRrcUrllcSapUser (it->first));
      ...
      //Ravi URLLC SAP Provider
      DynamicCast<ComponentCarrierEnb> (it->second)->GetMac ()->SetFfMacUrllcSchedSapProvider (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->GetFfMacSchedSapProvider ());
      ...
      //Ravi control for the URLLC schedular
      DynamicCast<ComponentCarrierEnb> (it->second)->GetMac ()->SetFfMacCschedUrllcSapProvider (DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->GetFfMacCschedSapProvider ());
      ...
      //Ravi URLLC SAP user
      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->SetFfMacSchedSapUser (DynamicCast<ComponentCarrierEnb> (it->second)->GetMac ()->GetFfMacUrllcSchedSapUser ());
      ...
      //SAP USER FOR urllc SCHEDULER
      DynamicCast<ComponentCarrierEnb> (it->second)->GetFfMacUrllcScheduler ()->SetFfMacCschedSapUser (DynamicCast<ComponentCarrierEnb> (it->second)->GetMac ()->GetFfMacCschedUrllcSapUser ());
...
/*void NotifyRbgAllocation (std::string context,uint16_t embbAllocation,  uint16_t urllcAllocation)
{
...
//void
//LteHelper::EnableRbgTraces (void)
//{
  //NS_LOG_FUNCTION_NOARGS ();
 // Config::Connect ("/NodeList/*/DeviceList/*/ComponentCarrierMap/*/LteEnbMac/RbgAllocation",MakeCallback (&NotifyRbgAllocation));
//}
