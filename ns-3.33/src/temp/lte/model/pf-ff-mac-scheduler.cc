...
void PfFfMacScheduler::DoCschedCellConfigReq (const struct FfMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  int rbgIndex=0;
  double percentageReserved=0.1;
  uint16_t blockedRbgs= GetBlockedRB();
  NS_LOG_INFO("Blocking RBGs for EMBB "<<blockedRbgs);
  //Block certain RBGs for URLLC
  for (std::vector<bool>::reverse_iterator it = rbgMap.rbegin (); it != rbgMap.rend(); it++){
  ...
  for (int i = 0; i < rbgNum; i++)
    {
      ...
      if (rbgMap.at (i) == false)
        ...
        else { //Ravi - to record even if there is no allocation

          NS_LOG_INFO("RECORDING THE CQI/MCS EVEN IF THERE IS NO ALLOCATION");
