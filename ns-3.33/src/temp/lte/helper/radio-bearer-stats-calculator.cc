    ...
    m_protocolType ("RLC")
{
  ...
  //Ravi for delay cdf
  embbDelayDataFile.open ("embbDelayfile.txt", std::ofstream::out|std::ofstream::trunc);
  urllcDelayDataFile.open ("urllcDelayfile.txt", std::ofstream::out|std::ofstream::trunc);
}
    ...
    m_pendingOutput (false)
{
  ...
  //Ravi for delay cdf
  embbDelayDataFile.open ("embbDelayfile.txt", std::ofstream::out);
  urllcDelayDataFile.open ("urllcDelayfile.txt", std::ofstream::out);
}

RadioBearerStatsCalculator::~RadioBearerStatsCalculator ()
{
  NS_LOG_FUNCTION (this);
  embbDelayDataFile.close();
  urllcDelayDataFile.close();
}
  ...
  if (Simulator::Now () >= m_startTime)
    {
      ...
      if (it == m_dlDelay.end ())
        {
          ...
          if(imsi ==1){
            embbDelayDataFile<<imsi<<" "<<(double)delay/pow(10,9)<<std::endl;
          }else if(imsi ==2){
            urllcDelayDataFile<<imsi<<" "<<(double)delay/pow(10,9)<<std::endl;
          }
        }
      ...
      if(imsi ==1){
            embbDelayDataFile<<imsi<<" "<<(double)delay/pow(10,9)<<std::endl;
           // std::cout<<" Embb delay "<<(double)delay/pow(10,9)<<std::endl;
          }else if(imsi ==2){
            urllcDelayDataFile<<imsi<<" "<<(double)delay/pow(10,9)<<std::endl;
          }
