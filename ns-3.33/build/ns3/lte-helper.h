...
//Ravi
/** 
   * Set the type of URLLC scheduler to be used by eNodeB devices.
   * 
   * \param type type of scheduler, must be a type name of any class
   *             inheriting from ns3::FfMacScheduler, for example:
   *             "ns3::PfFfMacScheduler"
   ...
   * Equivalent with setting the `Scheduler` attribute.
   */
  void SetUrllcSchedulerType (std::string type);
   ...
   * \return the scheduler type
   */
  std::string GetUrllcSchedulerType () const; 
...
private:
  ...
  /// Factory of MAC URLLC scheduler object.
  ObjectFactory m_schedulerUrllcFactory;
