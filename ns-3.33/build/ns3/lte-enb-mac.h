...
public:
  ...
  //Ravi URLLC Schedular

    void SetFfMacUrllcSchedSapProvider (FfMacSchedSapProvider* s);
  /**
   * \brief Get the scheduler SAP user
   * \return a pointer to the SAP user of the scheduler
   */
  FfMacSchedSapUser* GetFfMacUrllcSchedSapUser (void);
  /**
   * \brief Set the control scheduler SAP provider
   * \param s a pointer to the control scheduler SAP provider
   */
  void SetFfMacCschedUrllcSapProvider (FfMacCschedSapProvider* s);
  /**
   * \brief Get the control scheduler SAP user
   * \return a pointer to the control scheduler SAP user
   */
  FfMacCschedSapUser* GetFfMacCschedUrllcSapUser (void);
...
/** Ravi
   * TracedCallback signature for DL scheduling events.
   * \param [in] embbRbgAllocation;
   * \param [in] urllcRbgAllocation;
   */
  typedef void (* rbgAllocationTracedCallback) (const uint16_t embbRbgAllocation, const uint16_t urllcRbgAllocation, 
  const uint16_t embbHOl, const uint16_t urllcHOl);


//Ravi, blocked RB configuration for emBB and URLLC UEs

void SetBlockedUrllcRB(uint16_t blockedRb){
  urllcBlockedRb=blockedRb;
}

void SetBlockedEmbbRB(uint16_t blockedRb){
  embbBlockedRb=blockedRb;
}

uint16_t getBlockedUrllcdRB(){
  return urllcBlockedRb;
}

uint16_t  getBlockedEmbbRB(){
  return embbBlockedRb;
}

private:

//Ravi, blocked RB configuration for emBB and URLLC UEs
uint16_t urllcBlockedRb;
uint16_t embbBlockedRb;
  ...
  */
  ...
  /* Ravi: RB mangement function to determine the RBG size fo URLLC and Embb flows*/
  void ExecuteRBManagement();
...
private:
  ...
  */
  ...
  //Ravi URLLC m_dlCqiUrllcReceived
  std::vector <CqiListElement_s> m_dlCqiUrllcReceived; ///< DL-CQI received
  ...
  */
  ...
  FfMacCschedSapUser* m_cschedSapUser; ///< the CSched SAP user
   ...
   //Ravi: user this for URLLC
  FfMacSchedSapProvider* m_schedSapUrllcProvider; ///< the Sched SAP provider
  FfMacCschedSapProvider* m_cschedSapUrllcProvider; ///< the Csched SAP provider
  FfMacSchedSapUser* m_schedSapUrllcUser; ///< the Sched SAP user
  FfMacCschedSapUser* m_cschedSapUrllcUser; ///< the CSched SAP user
  ...
  /* Ravi Traced call back to track RBG allocation*/
  TracedCallback<uint16_t, uint16_t,uint16_t, uint16_t> rbgAllocation;
  ...
  struct averageStats{
    uint16_t rnti;
    uint32_t averageArrivalInBytes;
    uint32_t averageArrivalInPrb;
    uint32_t buffersizeInBytes;
    uint32_t buffersizeInPrb;
    uint32_t previousAllocation;
    int16_t latestMcs;
    int16_t latestTbs;
    int32_t latestRbBitMap;
    int16_t numberOfRbs;
    int32_t predictedRbs;
    uint32_t holDelay;
  };
  ...
  uint16_t dlBandwidth, ulBandwidth;
  ...
  //Ravi, map for RB management
  //std::map<uint16_t, averageStats> embbAvgStats;
  std::map<uint16_t, averageStats> userAvgStats;

  std::map<uint16_t, std::deque<arrivalData> > userStats;
  //std::map<uint16_t, std::deque<arrivalData> > urllcStats;

  //last update frm Schedular
  std::map<uint16_t, struct Schedularinfo> SchedlastUpdate;
