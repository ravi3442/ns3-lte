...
NS_LOG_COMPONENT_DEFINE ("test-sim");
 ...
 uint32_t ByteCounterNear = 0;
 uint32_t oldByteCounterNear= 0;

uint32_t ByteCounterFar = 0;
 uint32_t oldByteCounterFar= 0;
...
//---------------------------------# Added for Throughput Calculation
 ...
 void
 ReceivePacketNear (Ptr<const Packet> packet, const Address &)
 {
   ByteCounterNear += packet->GetSize ();
  // std::cout << Simulator::Now() << "  Packet Size " << packet->GetSize () << std::endl;
 }

void
 ReceivePacketFar (Ptr<const Packet> packet, const Address &)
 {
   ByteCounterFar += packet->GetSize ();
   //std::cout << Simulator::Now() << "  Packet Size " << packet->GetSize () << std::endl;
 }

 void
 ThroughputNear(bool firstWrite, Time binSize, std::string fileName)
 {
   std::ofstream output;
   ...
   double  throughput = (ByteCounterNear - oldByteCounterNear)*8/binSize.GetSeconds ()/1024/1024;
   ...
   oldByteCounterNear = ByteCounterNear;
   Simulator::Schedule (binSize, &ThroughputNear, firstWrite, binSize, fileName);
   std::cout<<"Throughput Near "<<throughput<<std::endl;
 }

 void
 ThroughputFar(bool firstWrite, Time binSize, std::string fileName)
 {
   std::ofstream output;
   ...
   //Instantaneous throughput every 200 ms
   double  throughput = (ByteCounterFar - oldByteCounterFar)*8/binSize.GetSeconds ()/1024/1024;
   //double  throughput = (ByteCounter)*8/Simulator::Now().GetSeconds ()/1024/1024;
   //double  throughput = (ByteCounter);
   output << Simulator::Now().GetSeconds() << " " << throughput << std::endl;
   oldByteCounterFar = ByteCounterFar;
   Simulator::Schedule (binSize, &ThroughputFar, firstWrite, binSize, fileName);
   std::cout<<"Throughput Far "<<throughput<<std::endl;
 }
...
//LtePdcp LteRlc LteRlcUm NoOpComponentCarrierManager
...
Ptr<UniformRandomVariable> coordinates = CreateObject<UniformRandomVariable> ();
  coordinates->SetAttribute ("Min", DoubleValue (-500));
...
for (uint16_t i = 0; i < numberOfEnbs; i++)
 ...
 for (uint16_t i = 0; i < numberOfUes/2; i++)
    {
      ...
      //std::cout<<"UE Node ID "<<ueNodes.Get(i)->GetId()<< ueNodes.Get(i)->GetNDevices().GetId()<< std::endl;
    //  c = i ; 
    }
...
for (uint32_t u = 0; u < numberOfUes; ++u)
    {
      ...
      if (ueNodes.Get (u)->GetId () == 6 ){
        ...
        Config::ConnectWithoutContext (oss0.str (), MakeCallback (&ReceivePacketFar));
      }

      if (ueNodes.Get (u)->GetId () == 5 ){

        oss1 << "/NodeList/" << ueNodes.Get (u)->GetId () << "/ApplicationList/0/$ns3::PacketSink/Rx";
        Config::ConnectWithoutContext (oss1.str (), MakeCallback (&ReceivePacketNear));
      }
...
bool firstWrite = true;
   ...
   std::string fileNameFar = "rlf_dl_thrput_" + std::to_string (enbNodes.GetN ()) + "_eNB_" + "12" + rrcType;
   std::string fileNameNear = "rlf_dl_thrput_" + std::to_string (enbNodes.GetN ()) + "_eNB_" + "5" + rrcType;
   Time binSize = Seconds (0.1);
 Simulator::Schedule (Seconds(0.1), &ThroughputNear, firstWrite, binSize, fileNameNear);
 Simulator::Schedule (Seconds(0.1), &ThroughputFar, firstWrite, binSize, fileNameFar);
