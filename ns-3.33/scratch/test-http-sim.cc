...
//-----------------------
  Config::SetDefault ("ns3::UdpClient::Interval", TimeValue (MicroSeconds (1000))); //128 Kbps make it 64000
...
//Ipv4Address serverAddress = remoteHost->GetObject<Ipv4>() //interfaces.GetAddress (1);
ThreeGppHttpServerHelper serverHelper (remoteHostAddr);

// Install HTTP server
ApplicationContainer serverAppsHttp = serverHelper.Install (remoteHost);
...
//--------------------------------------------------
NodeContainer ueHttpNodes(ueNodes.Get(0), ueNodes.Get(1));
...
for (int i=0; i<numberOfUes ; i++){
  httpClient = clientApps_http.Get (i)->GetObject<ThreeGppHttpClient> ();
      ...
      for (uint32_t b = 0; b < numBearersPerUe; ++b)
        {
         ...
         /* ApplicationContainer clientApps;
          ...
          //if(ue->GetId() == 5){
          //clientApps_http = clientHelper.Install (ueNodes.Get (u));
         // Ptr<ThreeGppHttpClient> httpClient = clientApps_http.Get (u)->GetObject<ThreeGppHttpClient> ();
        ...
        //}
          ...
          dlpf.localPortStart = 1024;
          dlpf.localPortEnd = 49000;
          tft->Add (dlpf);
          EpcTft::PacketFilter ulpf;
          ulpf.remotePortStart = 1024;
          ulpf.remotePortEnd = 49000;
          tft->Add (ulpf);
...
for (uint32_t u = 0; u < numberOfUes; ++u)
    {
     ...
     // oss1 << "/NodeList/" << ueNodes.Get (u)->GetId () << "/ApplicationList/0/$ns3::PacketSink/Rx";
      ...
      //oss3<<"/NodeList/*/DevicenList/*/LteUeRrc/DataRadioBearerMap/*/LtePdcp/RxPDU";
