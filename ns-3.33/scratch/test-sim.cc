...
LogLevel logLevel = (LogLevel)(LOG_PREFIX_FUNC | LOG_PREFIX_TIME | LOG_LEVEL_ALL);
 ...
 LogComponentEnable ("test-sim", logLevel);
...
Ptr<Node> pwg_http = epcHelper->GetPgwNode();
...
//create another Remote http server
NodeContainer remoteHttpHostContainer;
remoteHttpHostContainer.Create(1);
Ptr<Node> remoteHtppHost = remoteHttpHostContainer.Get(0);
...
NetDeviceContainer internetHttpDevices = p2ph.Install (pgw, remoteHtppHost);
...
//remoteHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("2.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);
// Routing of the Internet Host (towards the LTE network)
Ipv4StaticRoutingHelper ipv4RoutingHelper2;
Ptr<Ipv4StaticRouting> remoteHttpHostStaticRouting = ipv4RoutingHelper2.GetStaticRouting (remoteHtppHost->GetObject<Ipv4> ());
// interface 0 is localhost, 1 is the p2p device
//remoteHttpHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);


NS_LOG_INFO("Remote HTTP Server Ip Add: ");
remoteHostHttpAddr.Print(std::cout);
std::cout<<"\n";

NS_LOG_INFO("Remote PGW Ip Add: ");
Ipv4Address pGwAddr = internetIpHttpIfaces.GetAddress (0);

pGwAddr.Print(std::cout);
std::cout<<"\n";

ThreeGppHttpServerHelper serverHelper (remoteHostHttpAddr);

// Install HTTP server
ApplicationContainer serverAppsHttp = serverHelper.Install (remoteHtppHost);
...
   mobility.Install(remoteHtppHost);
   remoteHtppHost->GetObject<MobilityModel> ()->SetPosition (Vector (500+ coordinates->GetValue(),500+coordinates->GetValue(),0));
...
for (uint32_t u = 0; u < numberOfUes; ++u){
  ...
  if(ue->GetId()==5){
    ...
    NS_LOG_INFO("Http Server Address: ");
    remoteHostHttpAddr.Print(std::cout) ;
...
// Add X2 interface
 ...
 // connect custom trace sinks for RRC connection establishment and handover notification
  Config::Connect ("/NodeList/*/DeviceList/0/LteEnbRrc/ConnectionEstablished",
...
//--------------------------------------------------
  ...
  std::ostringstream oss0, oss1,oss3;
