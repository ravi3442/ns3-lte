...
//-----------------------
   Config::SetDefault ("ns3::UdpClient::Interval", TimeValue (MicroSeconds (64000))); //128 Kbps make it 64000
...
// Install Mobility Model in mobile UE
  ...
  mobility.SetPositionAllocator ("ns3::RandomDiscPositionAllocator",
                                 ...
                                 "Rho", StringValue ("ns3::ConstantRandomVariable[Constant=1000.0]"));
...
//int myRandomInt = xRandom->GetInteger ();
//double myRandomDbl = xRandom->GetValue ();

 Ptr<UniformRandomVariable> xRandom = CreateObject<UniformRandomVariable> ();
 double myRandomDbl = 0.0;
 double xvalue = 0.0;
 double yvalue = 0.0;
 for (uint16_t i = 0; i < 60; i++)
    {
      myRandomDbl = xRandom->GetValue ();
      xvalue=5000.0*myRandomDbl;
      myRandomDbl = xRandom->GetValue ();
      yvalue=5000.0*myRandomDbl;
      ueNodes.Get (i)->GetObject<MobilityModel> ()->SetPosition (Vector (xvalue,yvalue,0));
    }
...
//---------------------------------------------
   AnimationInterface anim ("test13.xml");
...
//flow monitor
//flowmon->SerializeToXmlFile ("test13.flowmon", true, true);
