...
//-----------------------
   Config::SetDefault ("ns3::UdpClient::Interval", TimeValue (MicroSeconds (32000))); //128 Kbps make it 64000
...
// Install Mobility Model in eNB
     ...
     Ptr<ListPositionAllocator> enbPositionAlloc = CreateObject<ListPositionAllocator> ();
          ...
          Vector enb1Position (1000, 500, 0);
          enbPositionAlloc->Add (enb1Position);

          Vector enb2Position (3000, 500, 0);
          enbPositionAlloc->Add (enb2Position);

          Vector enb3Position (2000, 2232.051, 0);
          enbPositionAlloc->Add (enb3Position); 
...
for (uint16_t i = c; i < numberOfUes; i++)
    {
      mobility.Install (ueNodes.Get(i));
    }
  ...
  for (uint16_t i = 0; i < 2; i++)
    {
     for (uint16_t j = 0; j < 5; j++)
      {
         c++;
         ueNodes.Get (c)->GetObject<MobilityModel> ()->SetPosition (Vector ((700+150*j),(200+500*i),0));
    ...
    for (uint16_t i = 0; i < 2; i++)
    {
     for (uint16_t j = 0; j < 2; j++)
      {
         c++;
         ueNodes.Get (c)->GetObject<MobilityModel> ()->SetPosition (Vector ((3200+150*j),(400+200*i),0));
    ...
    for (uint16_t i = 0; i < 1; i++)
    {
     for (uint16_t j = 0; j < 1; j++)
      {
         c++;
         ueNodes.Get (c)->GetObject<MobilityModel> ()->SetPosition (Vector ((2200+150*j),(2280+200*i),0));
...
for (uint16_t i = 6; i < numberOfUes; i++)
    {
...
//---------------------------------------------
   AnimationInterface anim ("test11.xml");
   anim.SetMaxPktsPerTraceFile(1000000); // abhishek
//---------------------------------------------
...
//flow monitor
//flowmon->SerializeToXmlFile ("test11.flowmon", true, true);
