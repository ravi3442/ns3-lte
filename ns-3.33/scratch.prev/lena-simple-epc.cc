...
static void printDataStore(Ptr<LteHelper> lteHelper){

	std::cout<<"***Content int the datastore***"<<std::endl;

	//lteHelper->utilizationStore.displayUlData();
	//lteHelper->utilizationStore.displayDlData();
...
main (int argc, char *argv[])
{
  ...
  for (uint16_t i = 0; i < numNodePairs; i++)
    {
      ...
      //Ptr<Node> eNodeB_p = enbNodes.Get(i);
      Ptr<NetDevice> temp = enbLteDevs.Get(i);
      Ptr<LteEnbNetDevice> enbLteDevice = temp->GetObject<LteEnbNetDevice>();

      std::cout<<"Printing Cell Id: "<<enbLteDevice->GetCellId ()<<std::endl;
  ...
  //set a timer to print the contents of the data store

  for(int i=0;i <simTime.GetMilliSeconds();i++){

	  Simulator::Schedule (Seconds (0.1*i), &printDataStore, lteHelper);
  }
